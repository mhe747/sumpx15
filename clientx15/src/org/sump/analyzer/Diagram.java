...
public class Diagram extends JComponent implements MouseMotionListener, Configurable {
	...
	public Diagram() {
		...
		MainWindow.currDiagram = this;
		
		//  ?new2 init
		MainWindow.waveViewEnabled. addActionListener(new ActionListener() {
	        @Override
	        public void actionPerformed(ActionEvent e) {
	           
	        	MainWindow.currDiagram.resize(); 
	        }
	    });
	  ...
	  //  ?new2 init
		MainWindow.spectrumViewEnabled. addActionListener(new ActionListener() {
		        @Override
		        public void actionPerformed(ActionEvent e) {
	...
	public void setCapturedData(CapturedData capturedData) {
		if (capturedData==null) {
			System.out.println("Error in setCapturedData");
			return;
		}
		...
		//samplingFreq = capturedData.rate/1000000;
	...
	private void drawEdge2(Graphics g, int x, int prevY, int currY, boolean falling, boolean rising) {
		if (scale <= 1) {
			g.drawLine(x, prevY, x, prevY + 14);
		} else {
			int edgeX = x;
			if (scale >= 5)
				edgeX += (int)(scale * 0.4);
			
			g.drawLine(x, prevY, edgeX, currY);
			g.drawLine(edgeX, currY, x + (int)scale, currY);
	 ...
	 * @param to index of last sample to display
	 */
	private void drawAnalogical(Graphics g, int x, int y, int[] data, int n, int from, int to, Rectangle clipArea) {
		
		int yShift = analogGridOff;
		int yOff = 0;
		//setComponentZOrder(this, 5);
		...
		for(int i=33; i>0; --i)
		{
		 if(i%3 == 0) {
		  g.setColor(text);
		  g.drawString(Float.toString((float) (i*0.1)), 5, y + yOff);
		  g.setColor(grid);
		  g.drawLine(clipArea.x, y + yOff +2, clipArea.x + clipArea.width, y + yOff +2);
		 }
		...
		for (int current = from; current < to;) {
			...
			do {
				nextV = data[++next];
			} while ((next < to) && (Math.abs(nextV - currentV)<150)); // scan for the next significant change
			int currentEndX = currentX + (int)(scale * (next - current - 1));
			// move currentX to the point before changing event
			int back_currentX = currentX;
			if (next>(current+2)) {
				currentX = currentEndX - (int)(scale);			
				int same_level =  y + (int)(analogWaveHeight * (1 - (float)currentV/scale_factor));
				g.drawLine(back_currentX, same_level, currentX, same_level);
			}
			
			
			/* slow draw, every segment is now represented 
			nextV = data[++next];
			int currentEndX = currentX + (int)(scale * (next - current - 1));
			*/
			
			int currY = (int) (analogWaveHeight * (1 - (float)currentV/scale_factor));
			int nextY = (int) (analogWaveHeight * (1 - (float)nextV/scale_factor));
			
			// draw straight line up to the point of change and a edge if not at end
			if (currentV == nextV) {
				g.drawLine(currentX, y + currY, currentEndX + (int)scale, y + currY);
			} else {
				g.drawLine(currentX,y + currY, currentEndX + (int)scale, y + nextY);
			}
	 ...
	 * @param to index of last sample to display
	 */
	private void drawSpectrum(Graphics g, int x, int y, int[] data, int n, int from, int to, Rectangle clipArea) {
		...
		double[] FFTAmpl = new double[data.length];	  // buffer with FFT amplitudes
		double maxFFTAmpl = 0;
		for(int i=0; i<data.length; ++i) {
			double ampl = Math.sqrt(real[i]*real[i] + imag[i]*imag[i]);
			FFTAmpl[i] = ampl;
			if(ampl > maxFFTAmpl)
				maxFFTAmpl = ampl;
		}
		
		//normalization of FFT amplitudes
		int[] FFTAmplPlot = new int[data.length];	  // buffer with FFT amplitudes
		for(int i=0; i<FFTAmpl.length; ++i)
			FFTAmplPlot[i] = (int) ((FFTAmpl[i]/maxFFTAmpl)*SpectrumHeight);
		
		int yShift = SpectrumHeight/100;
		int yOff = 0;
		//setComponentZOrder(this, 5);
		...
		for(int i=100; i>0; --i)
		{
		 if((i%10) == 0) {
		  g.setColor(text);
		  g.drawString(Float.toString((float) (i*0.01)), 5, y + yOff);
		  g.setColor(grid);
		  g.drawLine(clipArea.x, y + yOff +2, clipArea.x + clipArea.width, y + yOff +2);
		 }
		...
		yOff += 20;
		int yXAxisValeus = y + yOff;
		int xAxisUnit = FFTAmpl.length/20;
		g.drawString(Float.toString( (float) 0), x , yXAxisValeus);
		
		
		
		for (int current = from; current < to;) {
			int currentX = (int)(x + current * scale);
			int currentV = FFTAmplPlot[current];
			int nextV = currentV;
			int next = current;
			...
			// scan for the next change
			do {
				nextV = FFTAmplPlot[++next];
				
				    if((-(from-to))<10) {
				    	
				    	
				    	if(next == to) {
				    		int currentX2 = (int)(x + next * scale);
							g.setColor(text);
							float xVal =  ((float)((next+2)/(float)(FFTAmpl.length)))*samplingFreq;
							
							g.drawString(String.format("%.3f", xVal), currentX2 , yXAxisValeus);
							g.setColor(signal);		
				    	}
				    	
				    }
				    else{
				      if((next%((from-to)/10)) ==0) {
						int currentX2 = (int)(x + next * scale);
						g.setColor(text);
						float xVal =  ((float)((next+2)/(float)(FFTAmpl.length)))*samplingFreq;
			...
			int currY = (int) (SpectrumHeight * (1 - (float)currentV/SpectrumHeight));
			int nextY = (int) (SpectrumHeight * (1 - (float)nextV/SpectrumHeight));
	...
	public void paintComponent(Graphics g) {
		...
		if(MainWindow.waveViewEnabled.isSelected())   //? new2
		{
		...
		drawAnalogical(g, xofs, bofs  + analogWaveOff, data, 2, firstRow, lastRow, clipArea);
	    } //? new2
		else
		drawSpectrum(g, xofs, yofs, data, 2, firstRow, lastRow, clipArea);	
	}
	...
	final int analogGridOff = 4;
	final int analogWaveOff = 120;
	final int analogWaveHeight = 120;
	final int SpectrumHeight = 500;
	
	// 14 bits DAC
	public static final int maxIntegerInput = 16383;
	public static final int scale_factor = 16383;
	public static float samplingFreq = 50;  // sampling Frequency in Mhz, read from capturedData.rate
